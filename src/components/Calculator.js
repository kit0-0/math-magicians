...
function Calculator() {
  const renderButton = (text, name, color) => (
    <div className="flex-1 px-2 py-2 justify-center flex items-center text-white text-2xl font-semibold">
      <button
        type="button"
        className={`rounded-full h-12 w-20 flex items-center ${color} justify-center shadow-lg hover:border-gray-500 focus:outline-none`}
        name={name}
      >
        {text}
      </button>
    ...
    <div className="mx-auto overflow-hidden mt-10 shadow-lg mb-2 bg-[#a4a4a4] shadow-lg border rounded-lg lg:w-2/6 md:w-3/6 sm:w-4/6">
      <div className="">
        ...
        <div className="flex items-stretch bg-[#c8c8c8] h-20 mb-4">
          {renderButton('AC', null, 'bg-gray-800')}
          {renderButton('0', '0', 'bg-gray-800')}
          {renderButton('.', '.', 'bg-gray-800')}
          {renderButton('=', null, 'bg-orange-500')}
        </div>
      </div>
    </div>
  );
